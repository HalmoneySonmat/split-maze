...
- **3.2 🟡 코드 박제, WSL 검증 대기**: `src/split_maze/paired_collect.py` (~280줄, PairBufferConfig + PairBuffer FIFO ring + PairedCollector stride-based) + `tests/test_paired_collect.py` (~45 tests, 9 클래스). P3-2-1~P3-2-5 박제 (아래 §9.12 + §10.3).
...
### A5. ★ 전체 회귀 (~200 tests 기대 — acc 42 + paired_collect ~45 추가)
```bash
PYTHONPATH=src python -m pytest tests/ -q
```
기대: `~200 passed in X.Xs`. Phase 1·2 기존 회귀 없는지 확인.
