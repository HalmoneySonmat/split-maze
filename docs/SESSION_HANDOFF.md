...
- **3.3 진행 중**: `builds.py` — B3/B4/V2 wrapper + `adapter.py`. 세부:
  ...
  - **3.3.3 🟡 코드 박제 (2026-05-21), WSL 검증 대기**: `builds.py` `V2ACC` — lm.encode(ids) 재계산(P3-2-3) + ACC.recon_loss. LM core frozen, interface_proj만 trainable (P3-2-A). `test_builds.py` V2 +9. **WSL 검증 (§9.6 A9)**. → builds.py 3 빌드 전부 완성.
