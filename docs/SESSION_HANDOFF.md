...
### Phase 3 sub-단계
- **3.1 ✅ 코드+검증**: `src/split_maze/acc.py` (~280줄) + `tests/test_acc.py` (42 tests, WSL 2026-05-20 통과). ACCConfig + ACC (tied W + 양방향 LayerNorm + MSE + (C-thin) detach + cross_cosine eval). P3-1~P3-5 박제값 default.
- **3.2 🟡 spec 박제, 코딩 대기**: `src/split_maze/paired_collect.py` — RL rollout → (h_agent, h_lm) 페어 추출 + replay buffer. P3-2-1~P3-2-4 박제 (아래 §9.12).
- **3.3**: `src/split_maze/builds.py` — B3 (probe), B4 (어댑터+next-token), V2 (ACC) wrapper.
- **3.4**: 공동 학습 루프 (P3-1 α + P3-5 동시 부착).
...
### 다음 세션 진입 명령 (이후 박제)
"Phase 3.2 — paired_collect.py 코딩 진입. P3-2-1~P3-2-4 spec §9.12 그대로,
train_phase3.py 분리 여부만 추가 결정."
...
WSL 1차 검증 후 사용자가 이 해석에 동의하면 PLAN §4.2 본문에 명시적 각주로 박제 권장.
